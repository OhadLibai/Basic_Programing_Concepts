...
public class FileIndexTester {
	...
	public static void main(String[] args) throws IOException, FileIndexException {
	...
	public static void testTFIDF() throws IOException, FileIndexException {
	...
	public static void testSortingBySignificance() throws IOException, FileIndexException {
		FileIndex fIndex = new FileIndex();
		...
		try{
			...
			TesterUtils.checkDoubleValueAndPrintError(0.0931, res.get(0).getValue(), EPS, 27);
			TesterUtils.checkDoubleValueAndPrintError(0.0446, res.get(1).getValue(), EPS, 28);
			TesterUtils.checkDoubleValueAndPrintError(0.0446, res.get(2).getValue(), EPS, 28);

			res = fIndex.getTopKMostSignificantWords("something.txt", 3);
			if (res.size() != 3){
	...
	public static void testCosineSimilarity() throws IOException, FileIndexException {
		FileIndex fIndex = new FileIndex();
		...
		try{
			...
			//tf-idf vec for f1 = [a=0.1386, b=0.2773, c=0.0575, d=0]
			//tf-idf vec for f2 = [a=0, b=0.2773, c=0.1151, d=0]
			double sim = fIndex.getCosineSimilarity("f1.txt", "f2.txt");

			//System.out.println(fIndex.getTopKMostSignificantWords("f1.txt", 4) + " this was f1" + "\n");
			//System.out.println(fIndex.getTopKMostSignificantWords("f2.txt", 3) +  " this was f2" +"\n");
	...
	public static void testSortingByCosineSimilarity() throws IOException, FileIndexException {
		FileIndex fIndex = new FileIndex();
		fIndex.indexDirectory(INPUT_FOLDER_2);
		try{
